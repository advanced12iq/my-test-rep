...
public:
    ...
    GradientDescentOptimizerTracking(
        ...
        learning_rate(lr), min_value(min_val), max_value(max_val),
    ...
    std::pair<std::vector<double>, int> optimize() {
        ...
        for (int iter = 0; iter < max_iterations; iter++) {
            ...
            for (int coord = 0; coord < dimension; coord++) {
                ...
                if (fitness_decrease < std::min(current_fitness, fitness_increase)) {
                    x[coord] -= step_size;
                    current_fitness = fitness_decrease;
                } else if (fitness_increase < std::min(current_fitness, fitness_decrease)) {
                    x[coord] += step_size;
...
int main() {
    std::cout << "Fixed Iterations Optimization Test (1000 iterations)" << std::endl;
    std::cout << "====================================================" << std::endl;
    ...
    saveStepsToCSV(all_steps, "fixed_iterations_results.csv");
